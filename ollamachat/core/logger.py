"""Simple file logger for OllamaChat.

Writes log entries to ``data/ollamachat.log``. Never raises: logging
...
_LOG_DIRNAME = "data"
_LOG_FILENAME = "ollamachat.log"
_LOGGER_NAME = "ollamachat"


def get_logger(name: str = _LOGGER_NAME) -> logging.Logger:
    """Return a configured logger that writes to ``data/ollamachat.log``.
    ...
    Uses a sentinel attribute (``_ollamachat_configured``) on the
    ...
    Args:
        name: Logger name. Defaults to ``"ollamachat"``.

    Returns:
        A configured ``logging.Logger`` instance.
    """
    logger = logging.getLogger(name)
    if getattr(logger, "_ollamachat_configured", False):
        return logger

    logger.setLevel(logging.DEBUG)
    logger.propagate = False
    logger._ollamachat_configured = True  # type: ignore[attr-defined]
